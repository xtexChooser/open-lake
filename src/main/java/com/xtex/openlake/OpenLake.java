...
public final class OpenLake implements ModInitializer, ClientModInitializer {
    ...
    public static final String MOD_ID = "open-lake";
    ...
    public static final ItemGroup ITEM_GROUP = FabricItemGroupBuilder.create(id("item_group"))
            .icon(() -> new ItemStack(RetreatSwordItem.ITEM, shouldEnableEasterEgg(2) ? 18 : 1))
            .build();

    static {
        var mod = FabricLoader.getInstance().getModContainer(MOD_ID);
        if (mod.isPresent()) {
            LOGGER.info("Open-Lake v{}", mod.get().getMetadata().getVersion().getFriendlyString());
        } else {
            LOGGER.warn("Open-Lake works without '{}' mod id", MOD_ID);
        }
