package com.xtex.openlake.config;
...
public class OpenLakeConfig {
    ...
    private static final File FILE = FabricLoader.getInstance().getConfigDir().resolve("open_lake.json").toFile();
    ...
    public static Screen createConfigScreen(Screen parent) {
        var builder = ConfigBuilder.create()
                ...
                .setTitle(new TranslatableText("openlake.config"))
                .setSavingRunnable(OpenLakeConfig::write);
        builder.getOrCreateCategory(new TranslatableText("openlake.config.general"))
                .addEntry(builder.entryBuilder()
                        .startBooleanToggle(new TranslatableText("openlake.config.general.slim"), INSTANCE.useSlimModel)
                        .setDefaultValue(false)
                        .setSaveConsumer((value) -> INSTANCE.useSlimModel = value)
                        .requireRestart()
                        .build())
                .addEntry(builder.entryBuilder()
                        .startBooleanToggle(new TranslatableText("openlake.config.general.easter_eggs"), INSTANCE.enableEasterEggs)
                        .setDefaultValue(true)
                        .setSaveConsumer((value) -> INSTANCE.enableEasterEggs = value)
                        .requireRestart()
                        .build());
        builder.getOrCreateCategory(new TranslatableText("openlake.config.devtools"))
